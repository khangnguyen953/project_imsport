...
const Cart = () => {
    ...
    const handleQuantityChange = (id, variationId, value) => {
        updateQuantity(id, variationId, value);
    }
    const handleDelete = async (id, variationId) => {
        if (userId) {
            const payload = {
                user_id: userId,
                product_id: id,
    ...
    const handleUpdateCart = async (id, variationId, value) => {
        if (value !== '' && value > 0) {
            updateQuantity(id, variationId, value);
            console.log('cart sau khi update', cart.find(item => item.id === id && item.variationId === variationId));
            if (userId) {
                const payload = {
                    user_id: userId,
                    product_id: id,
                ...
                const response = await CartAPI.updateCart(userId, payload);
                console.log('response', response);
            }
        }
    }
                                                ...
                                                <div className='md:flex md:w-[35%] w-[20%] items-center'>
                                                    <div className='text-center md:w-[30%] w-full'>
                                                        <input type="number" min="1" className='md:w-20 md:h-10 w-12 h-7 px-2 border-[1px] border-[#ddd] md:rounded-full  text-[#363636] text-center outline-none' onFocus={(e) => e.target.select()} defaultValue={1} value={item.quantity === 0 ? '' : item.quantity}
                                                            onBlur={(e) => handleUpdateCart(item.id, item.variationId, e.target.value === '' || Number(e.target.value) <= 0 ? 0 : Number(e.target.value))}
